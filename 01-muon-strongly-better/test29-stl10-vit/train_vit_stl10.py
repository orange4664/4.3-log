...
def get_stl10(data_dir: str, download: bool):
    import torchvision
    import torchvision.transforms as T

    train_tf = T.Compose(
        [
            T.RandomCrop(96, padding=8),
    ...
    train = torchvision.datasets.STL10(root=data_dir, split="train", download=download, transform=train_tf)
    test = torchvision.datasets.STL10(root=data_dir, split="test", download=download, transform=test_tf)
    return train, test
...
def train_one(args: argparse.Namespace, seed: int, lr: float, method: str, outdir: Path) -> Dict[str, float]:
    ...
    train_set, test_set = get_stl10(args.data_dir, args.download)
    model = TinyViT(
        image_size=96,
