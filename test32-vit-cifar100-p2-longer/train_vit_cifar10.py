...
def get_cifar100(data_dir: str, download: bool):
    import torchvision
    ...
    train = torchvision.datasets.CIFAR100(root=data_dir, train=True, download=download, transform=train_tf)
    test = torchvision.datasets.CIFAR100(root=data_dir, train=False, download=download, transform=test_tf)
    return train, test
...
def train_one(args: argparse.Namespace, seed: int, lr: float, method: str, outdir: Path) -> Dict[str, float]:
    ...
    train_set, test_set = get_cifar100(args.data_dir, args.download)
    model = TinyViT(
        image_size=32,
