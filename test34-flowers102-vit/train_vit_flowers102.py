...
def get_flowers102(data_dir: str, download: bool):
    import torchvision
    ...
    train = torchvision.datasets.Flowers102(root=data_dir, split="train", download=download, transform=train_tf)
    val = torchvision.datasets.Flowers102(root=data_dir, split="val", download=download, transform=train_tf)
    test = torchvision.datasets.Flowers102(root=data_dir, split="test", download=download, transform=test_tf)
...
def train_one(args: argparse.Namespace, seed: int, lr: float, method: str, outdir: Path) -> Dict[str, float]:
    ...
    train_set, test_set = get_flowers102(args.data_dir, args.download)
    model = TinyViT(
        image_size=224,
...
def main() -> None:
    ...
    ap.add_argument("--lrs", default="0.0003,0.0005")
