...
def get_cifar10(data_dir: str, download: bool):
    import torchvision
    ...
    train = torchvision.datasets.CIFAR10(root=data_dir, train=True, download=download, transform=train_tf)
    test = torchvision.datasets.CIFAR10(root=data_dir, train=False, download=download, transform=test_tf)
    return train, test
...
def train_one(args: argparse.Namespace, seed: int, lr: float, method: str, outdir: Path) -> Dict[str, float]:
    ...
    train_set, test_set = get_cifar10(args.data_dir, args.download)
    model = TinyMLPMixer(
        image_size=32,
    ...
    rec_path = method_dir / "records.csv"
    if records:
...
def main() -> None:
    ...
    ap.add_argument("--channel-dim", type=int, default=512)
