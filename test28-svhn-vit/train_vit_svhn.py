...
def get_svhn(data_dir: str, download: bool):
    import torchvision
    ...
    train = torchvision.datasets.SVHN(root=data_dir, split="train", download=download, transform=train_tf)
    test = torchvision.datasets.SVHN(root=data_dir, split="test", download=download, transform=test_tf)
    return train, test
...
def train_one(args: argparse.Namespace, seed: int, lr: float, method: str, outdir: Path) -> Dict[str, float]:
    ...
    train_set, test_set = get_svhn(args.data_dir, args.download)
    model = TinyViT(
        image_size=32,
